...
class ModuleSerializer(serializers.HyperlinkedModelSerializer):
    ...
    attributes = AttributeSerializer(many=True, read_only=True, source='attribute_values')
    ...
    class Meta:
        model = Module
        fields = [
            'url', 'id', 'type', 'mutator', 'creator', 'original', 'attributes',
            'type_id', 'item_id'
        ]
...
class ModuleAppraisalView(generics.GenericAPIView, mixins.RetrieveModelMixin):
    ...
    def get(self, request, pk):
        ...
        try:
            return Response(AppraisalSerializer(PricePredictor.predict_price(module)).data)
        except Exception:
