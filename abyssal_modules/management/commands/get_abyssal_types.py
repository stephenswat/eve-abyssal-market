...
from abyssal_modules.models.modules import ModuleType, StaticModule, DERIVED_ATTRIBUTES
...
class Command(BaseCommand):
    ...
    def import_types(self):
        for i in tqdm(ITEMS):
            ...
            items = ESI.request(
                "get_universe_types_type_id", client=self.client, type_id=i["normal_id"]
    ...
    def repair_attributes(self):
        ModuleDogmaAttribute.objects.filter(id=204).update(name="Rate of Fire Bonus")

        ModuleDogmaAttribute.objects.filter(id=2267).update(
            name="Capacitor Warfare Resistance"
        )

        ModuleDogmaAttribute.objects.filter(id=10974).update(name="Hull EM Resistance")
        ...
        TypeAttribute.objects.filter(
            attribute__id__in=[10974, 10975, 10976, 10977]
        ).update(high_is_good=True)

    def import_mutators(self):
        data = requests.get(
            "http://sde.hoboleaks.space/tq/dynamicitemattributes.json"
        ).json()
        ...
        for attr_id, data in DERIVED_ATTRIBUTES.items():
            ...
            for type_id in data["types"]:
                TypeAttribute.objects.update_or_create(
                    type_id=type_id,
                    attribute=attr,
                    defaults={"display": True, "high_is_good": data["high_is_good"]},
                )
    ...
    def create_static_types(self):
        ...
        for abyssal_id, sources in tqdm(mod_to_source.items()):
            for s in sources:
                data = ESI.request("get_universe_types_type_id", type_id=s).data

                module, _ = StaticModule.objects.update_or_create(
                    id=s, defaults={"source_id": s, "type_id": abyssal_id}
                )
                    ...
                    try:
                        ModuleAttribute.objects.update_or_create(
                            ...
                            new_attribute=TypeAttribute.objects.get(
                                type_id=abyssal_id, attribute_id=attr["attribute_id"]
                            ),
